...
void
on_StatusIcon_activate (GtkStatusIcon * status_icon, gpointer user_data)
{
	gnomegadu_tray_show_hide_main_window ();
}

void
on_StatusIcon_popupmenu (GtkStatusIcon * status_icon, guint button, guint activate_time, gpointer user_data)
{
	GtkWidget *main_window = glade_xml_get_widget (gladexml, "MainWindow");
...
static
gboolean gnomegadu_tray_blinking_off (gpointer data)
{
...
void
gnomegadu_tray_blinking (int time)
{
	gtk_status_icon_set_blinking (status_icon, TRUE);
	g_timeout_add (time, gnomegadu_tray_blinking_off, NULL);
}
