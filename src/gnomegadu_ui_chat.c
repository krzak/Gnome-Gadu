	...
	if (gtk_tree_selection_count_selected_rows (selection) > 0)
	{
		...
		while (reference_list)
		{
			...
			if (gtk_tree_row_reference_valid (reference))
			{
				...
				gtk_tree_model_get (GTK_TREE_MODEL (contacts_tree_store), &iter, UI_CONTACTS_COLUMN_IS_GROUP, &is_group, -1);
...
static 
void gnomegadu_ui_chat_free_uin_list(gpointer list)
{
...
void gnomegadu_ui_chat_window_configure(GladeXML *chat_window_xml, GList *uin_list)
{
	...
	g_object_set_data_full(G_OBJECT(chat_window_xml),"html_str",g_strdup(""),g_free);
		...
		/* setup window */
			...
			column_icon = gtk_tree_view_column_new_with_attributes ("ikonka", render_pixbuf, "pixbuf", UI_CHAT_COLUMN_ICON, NULL);
			gtk_tree_view_append_column (GTK_TREE_VIEW (chat_window_tree_view), column_icon);

			column_name = gtk_tree_view_column_new_with_attributes ("Uczestnicy", render_text, "markup", UI_CHAT_COLUMN_DISPLAYED, NULL);
			...
			gnome_appbar_push (GNOME_APPBAR (status_bar), g_strdup_printf ("Ilość uczestników konferencji: %d", g_list_length (uin_list)));
...
gnomegadu_ui_chat_append_text(GladeXML *chat_window_xml, const gchar *txt, GnomeGaduChatType type, gchar *uin_sender_str)
{
	...
	{
	    html_str = g_strdup_printf("<table width='100%' bgcolor='#e5e5e5'><tr><td><b>Ja</b></td></tr><tr><td>%s</td></tr></table><br>",
				       escaped_txt);
	...
	else if (type == GNOMEGADU_CHAT_RCV)
	{
	    ...
	    html_str = g_strdup_printf("<table width='100%' bgcolor='#b7dbff'><tr><td><b>%s</b></td></tr><tr><td>%s</td></tr></table><br>",
